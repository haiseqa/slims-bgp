...
class Visitor
{
    ...
    public function record($memberId)
    {
        ...
        try {
            ...
            // cek pengunjung
            // cek pengunjung hari ini
            $today = date('Y-m-d');

            if ($this->member) {
                $checkQuery = $db->prepare("SELECT COUNT(*) FROM visitor_count WHERE member_id = ? AND DATE(checkin_date) = ?");
                $checkQuery->execute([$memberId, $today]);
            } else {
                $institution = trim($_POST['institution']);
                $checkQuery = $db->prepare("SELECT COUNT(*) FROM visitor_count WHERE member_name = ? AND institution = ? AND DATE(checkin_date) = ?");
                $checkQuery->execute([$memberId, $institution, $today]);
            }

            $alreadyCheckInToday = $checkQuery->fetchColumn() > 0;

            if ($alreadyCheckInToday) {
                $this->result = false;
                $this->alreadyCheckIn = true;
                $this->error = 'Mohon maaf, sebelumnya anda sudah melakukan registrasi kunjungan ke perpustakaan Mandala Widya Loka.. Terimakasih...';
                return $this; // <== ini penting biar proses insert di bawah gak jalan
            }       
